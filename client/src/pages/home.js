  ...
  onSubmit = e => {
    ...
    API.InsertIncompleteUrlEntry(url).then(res => {
      if (!res.data.success) {
        ...
        const urlHash = BijectiveHash.encode(id);
        API.UpdateShortenedUrl(id, urlHash).then(res => {
          if (!res.data.success) {
            this.setState({ error: res.error.message || res.error });
          } else {
            this.setState({
              message: `${window.location.protocol}//${window.location.host}/${urlHash}`
            });
          }
        });
      }
    ...
    return (
      <div>
        ...
        <div className="container">
          ...
          <div className="form">
            <ShortenForm url={this.state.url} handleChangeText={this.onChangeText} handleSubmit={this.onSubmit} />
          </div>
        </div>
      </div>
    );
